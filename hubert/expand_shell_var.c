...
issue #1 (2024 JUL 17)
	echo '$USER'$HOME -> echo 'jsu'$HOME // after ONE call of expand_shell_var() is incorrect
	echo $USER$HOME -> echo jsu$HOME // after ONE call of expand_shell_var() is correct

	to resolve:
	include a flag var, to track number of single quote before $ is found
	if ODD numbers of 'single quote char' found, then DO NOT EXPAND
	if EVEN numbers of 'single quote char' found, then expand shell var
	
issue #2 ()
	how to keep expanding, one shell var at a time, when there are multiple shell vars?
	remember, need to free the return value of expand_dollar_sign()!
*/
...
Note. the caller has to free the returned value!
*/
char	*expand_dollar_sign(char *s, char **env_ptr)
{
	char	*dollar;
	char	*last_ch;
	char	*expanded;
...
int	main(int ac, char **av, char** env)
{
	...
	printf("bef expansion:\n%s\n\n", input);
	
	output = expand_dollar_sign(input, env);
